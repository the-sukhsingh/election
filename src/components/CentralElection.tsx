...
import {
  ...
  ChevronLeft, BookOpen, Landmark, X
} from 'lucide-react';
...
const PHASES: Phase[] = [
  { 
    id: 'trigger',    
    title: 'The Signal',      
    subtitle: 'ECI Announcement',    
    icon: <Flag className="w-4 h-4" aria-hidden="true" />,        
    accent: '#FF6B35',
    description: "Every 5 years, the Election Commission triggers the world's largest democratic exercise, dividing India into 543 constituencies.",
    factTitle: "Independent Body",
    factDesc: "The ECI is constitutional and cannot be influenced by the ruling government."
  },
  { 
    id: 'candidates', 
    title: 'Candidates',      
    ...
    accent: '#3b82f6',
    description: "Political parties nominate candidates. Independent citizens can also run. Each aims to represent their local constituency in the Lok Sabha.",
    factTitle: "Requirements",
    factDesc: "Must be an Indian citizen, at least 25 years old, with public financial/criminal affidavits."
  },
  { 
    id: 'campaign',   
    title: 'Campaign',        
    subtitle: 'Public Rallies',      
    icon: <Megaphone className="w-4 h-4" aria-hidden="true" />,   
    accent: '#F59E0B',
    description: "Candidates campaign to win hearts. They hold rallies, participate in debates, and release manifestos detailing their promises.",
    factTitle: "Code of Conduct",
    factDesc: "The ruling party cannot announce new schemes. Campaigns halt 48h before polling."
  },
  { 
    id: 'voting',     
    title: 'Voting Day',      
    ...
    accent: '#22c55e',
    description: "Citizens cast their secret ballot using an Electronic Voting Machine (EVM). It's a simple button press that shapes the nation.",
    factTitle: "VVPAT",
    factDesc: "A printed slip shows your vote through a glass window for 7 seconds before dropping into a box."
  },
  { 
    id: 'results',    
    title: 'Results',         
    subtitle: 'Govt Formation',      
    icon: <BarChart2 className="w-4 h-4" aria-hidden="true" />,   
    accent: '#A855F7',
    description: "Votes are tallied. The party or coalition securing at least 272 seats wins the majority and forms the Central Government.",
    factTitle: "Coalition Govt",
    factDesc: "If no party hits 272 alone, multiple parties ally to form a coalition government."
  },
];
...
const ResultsPhase = memo(function ResultsPhase() {
  const winner = PARTIES[0]; // Derived safely from data in a real app, here hardcoded to index 0.
  return (
...
export default function CentralElection({ onBack }: { onBack?: () => void }) {
  ...
  return (
    <div className="w-full h-full flex flex-col relative text-white bg-transparent" aria-label="Central Election Educational Flow">
      {/* Minimal Header */}
      <header className="shrink-0 pt-2 pb-12 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Landmark className="w-5 h-5 text-gray-500" aria-hidden="true" />
          <h2 className="text-sm font-semibold text-gray-300 uppercase tracking-widest">Central Govt</h2>
        </div>
        <div className="flex-1 px-8 hidden md:block">
          <StepBar phases={PHASES} current={idx} />
        </div>
        <button onClick={onBack} aria-label="Close Central Election Module" className="p-2 rounded-full hover:bg-white/5 transition-colors text-gray-500 hover:text-white focus:outline-none focus:ring-2 focus:ring-white">
          <X className="w-5 h-5" />
        </button>
      </header>
